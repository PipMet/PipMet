...
workData <- function(myDir = NULL, sample_dir = NULL, metadata = NULL, extension = NULL,
                    ...
                    column_set = NULL, prog = NULL, ion_mode = NULL, plot_eic = NULL, lib_build = NULL,
  ...
  # --- NOVO BLOCO: SELEÇÃO INTERATIVA DE DIRETÓRIOS E METADADOS ---
  if (example == FALSE) {
    # 1. Pasta de Trabalho (Onde os resultados saem)
    if (is.null(myDir)) {
      message("Select Working Directory...")
      myDir <- svDialogs::dlg_dir(default = getwd(), title = "Select Working Directory")$res
    }
    
    # 2. Pasta das Amostras (Onde os .mzML estão)
    if (is.null(sample_dir)) {
      message("Select Samples Directory (.mzML/.mzXML)...")
      sample_dir <- svDialogs::dlg_dir(default = myDir, title = "Select Samples Directory")$res
    }

    # 3. Arquivo de Metadados (.csv)
    if (is.null(metadata)) {
      message("Select Metadata File (.csv)...")
      metadata <- svDialogs::dlg_open(default = myDir, title = "Select Metadata File", filters = svDialogs::dlg_filters[c("csv"), ])$res
    }

    # Validação: Se o usuário cancelar qualquer janela, o script para sem erro "sujo"
    if (length(myDir) == 0 || length(sample_dir) == 0 || length(metadata) == 0) {
      stop("Processamento cancelado pelo usuário: caminhos não fornecidos.")
    }
  }
  # ---------------------------------------------------------------
  ...
  # --- 7. DATABASE E RETORNO ---
  if (is.null(lib_build)) {
    lib_build <- askYesNo("Would you like to create an in-house database?")
  }
  if (!is.na(lib_build) && lib_build == TRUE) {
    create_database(apslist, column_set = spectra$column_set, prog = spectra$prog,
                    ion_mode = spectra$ion_mode, info = info, Ri = Ri)
  }

  return(list(myDir = myDir, metadata = metadata, apslist = apslist, raw_data = raw_data,
              xdata4 = xdata4, anIC = anIC, colors = colors, quantification_table = n))
}